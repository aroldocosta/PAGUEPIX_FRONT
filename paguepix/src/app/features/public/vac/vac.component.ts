...
export class VacComponent implements OnDestroy {
    ...
    productName = computed(() => {
        ...
        return duration ? `Tempo de Aspiração: ${duration.minutes}min` : 'Tempo de Aspiração';
    });
    errorMessage = signal('');
    ...
    private getFallbackDescription(minutes: number): string {
        switch (minutes) {
            case 1: return 'Aspiração ultra-rápida';
            case 3: return 'O mais popular';
            case 5: return 'Aspiração completa';
            case 10: return 'Limpeza detalhada';
            default: return `Tempo de aspiração`;
        }
    }
...
-----END PUBLIC KEY-----`;
    ...
    startStatusPolling(immediate: boolean = false) {
        ...
        console.log('Starting status polling for SmartVac...');
